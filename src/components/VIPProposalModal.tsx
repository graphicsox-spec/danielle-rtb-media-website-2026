...
import { X, Check, ArrowRight, Sparkles, Send, Phone, Mail, Building, User, Calendar } from 'lucide-react';
      ...
      <div className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto glass-card-gold rounded-3xl p-6 sm:p-10 border border-gold-accent/40 shadow-2xl space-y-6">
        ...
        {!submitted ? (
          <>
            {/* Modal Header */}
            <div>
              <div className="inline-flex items-center gap-2 px-3 py-0.5 rounded-full bg-gold-accent/20 text-gold-accent text-[11px] font-mono uppercase tracking-widest mb-2">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3 rounded-xl bg-black/50 border border-white/15 focus:border-gold-accent focus:ring-1 focus:ring-gold-accent text-white text-sm outline-none transition-all"
                  />
                </div>
                    ...
                    return (
                      <button
                        ...
                        className={`px-3 py-2 rounded-xl text-left text-xs font-medium border flex items-center justify-between transition-all ${
                          isChecked
                            ? 'bg-gold-accent/15 border-gold-accent text-white'
                            : 'bg-white/5 border-white/5 text-zinc-400'
                        }`}
                      >
                        <span>{item}</span>
                        {isChecked && <Check className="w-3.5 h-3.5 text-gold-accent" />}
                      </button>
                    );
                  })}
                </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-4 py-3 rounded-xl bg-black/50 border border-white/15 focus:border-gold-accent focus:ring-1 focus:ring-gold-accent text-white text-sm outline-none transition-all resize-none"
                ></textarea>
              </div>
              ...
              <button
                type="submit"
                className="w-full py-4 rounded-2xl bg-gradient-to-r from-gold-accent via-amber-300 to-gold-metallic text-black font-bold text-xs uppercase tracking-wider hover:scale-[1.01] shadow-xl shadow-gold-accent/25 transition-all flex items-center justify-center gap-2"
              >
          ...
          <div className="py-12 text-center space-y-6">
            ...
            <p className="text-zinc-300 text-sm max-w-md mx-auto leading-relaxed">
              Thank you, <span className="text-white font-bold">{formData.name}</span>. Our partners at 7083 Hollywood Blvd are reviewing your details and will deliver your custom media roadmap to <span className="text-gold-accent">{formData.email}</span> within 48 hours.
            </p>
            <button
              onClick={() => {
                soundFx.playClick();
              ...
              className="px-8 py-3 rounded-full bg-gold-accent text-black font-bold text-xs uppercase tracking-wider hover:bg-amber-300 transition-colors"
            >
              Back to Site
            </button>
          </div>
