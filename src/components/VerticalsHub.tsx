...
import { Cpu, Leaf, Palette, Trophy, ArrowRight, CheckCircle2, Star, Sparkles } from 'lucide-react';
...
export const VerticalsHub: React.FC<VerticalsHubProps> = ({ activeCategory = 'tech', onSelectCategory, onOpenProposal }) => {
  ...
  return (
    <section id="verticals" className="py-24 bg-[#09090E] relative border-t border-white/5">
          ...
          {VERTICALS.map((v) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-3 p-4 sm:p-5 rounded-2xl text-left transition-all duration-300 ${
                  isSelected
                    ? 'bg-gradient-to-br from-gold-accent/20 to-amber-500/10 border-2 border-gold-accent shadow-xl shadow-gold-accent/10 text-white'
                    : 'glass-card hover:border-white/20 text-zinc-300 hover:text-white'
                }`}
              >
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 transition-colors ${
                  isSelected ? 'bg-gold-accent text-black font-bold' : 'bg-white/5 text-gold-accent'
                }`}>
                  <Icon className="w-5 h-5" />
                </div>
                <div className="min-w-0">
                  <div className="text-xs uppercase font-mono tracking-wider text-zinc-400 truncate">{v.badge}</div>
                  <div className="font-display font-bold text-sm sm:text-base truncate">{v.title.split(' ')[0]}</div>
                </div>
              </button>
            );
          })}
        ...
        <div className="glass-card-gold rounded-3xl p-6 sm:p-10 lg:p-12 relative overflow-hidden border border-gold-accent/30 shadow-2xl">
          ...
          <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-10 items-center">
            <div className="lg:col-span-7 space-y-6">
              <div className="inline-block text-xs font-mono uppercase tracking-widest px-3 py-1 rounded-full bg-gold-accent/20 text-gold-accent border border-gold-accent/30">
              ...
              <div className="pt-4">
                <button
                  ...
                  onMouseEnter={() => soundFx.playHover()}
                  className="px-6 py-3 rounded-full bg-gold-accent text-black font-bold text-xs uppercase tracking-wider hover:bg-amber-300 shadow-lg shadow-gold-accent/20 transition-all flex items-center gap-2"
                >
            ...
            <div className="lg:col-span-5 space-y-4">
              <div className="glass-card rounded-2xl p-6 border border-white/10 space-y-6">
                <div className="text-xs uppercase font-mono tracking-widest text-gold-accent flex items-center gap-2">
